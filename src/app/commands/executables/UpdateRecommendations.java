...
public final class UpdateRecommendations implements Executable {
  @Override
  public Node execute(final InputNode command) {
    ...
    RecommendationType type =
        RecommendationType.valueOf(command.getRecommendationType().toUpperCase());
    recommendation =
        switch (type) {
          case RANDOM_SONG -> new SongRecommendationStrategy().getRecommendation(user);
          case RANDOM_PLAYLIST -> new RandomPlaylistRecommendationStrategy()
    ...
    if (recommendation == null) {
      return new UpdateRecommendationsOutputNode(command, "No new recommendations were found");
    }
    ...
    return new UpdateRecommendationsOutputNode(
        command,
        "The recommendations for user " + user.getUsername() + " have been updated successfully.");
  }
